...
/** Defines the known types of SVG icons available in the component. */
...
/** Props for the Svg component, extending standard SVG attributes. */
interface SvgComponentProps extends SVGProps<SVGSVGElement> {
    /** The key identifying which icon to render. */
    type: IconType;
    /** Whether the icon should apply an 'active' style override. */
    active?: boolean;
    /** If true, uses the default (red-coral) active/hover style; otherwise, uses sky-600. */
    defaultStyle?: boolean;
}

// --- Icon Definitions Map ---
/** * Maps IconType strings to their corresponding SVG paths/elements.
 * ReactElement
 */
const svgTypes: Record<IconType, ReactElement> = {
    moon: (
        <path
            strokeLinecap="round"
...
/**
 * A reusable SVG icon component that renders paths based on the 'type' prop.
 * * It automatically handles hover and active states using Tailwind CSS classes,
 * applying either the default red-coral theme or a sky-blue accent.
 * * @param {SvgComponentProps} props The component props.
 */
export const Svg: FC<SvgComponentProps> = ({
    type,
    className = "", // Use empty string as default for optional className
    onClick,
    active,
    defaultStyle = true,
    ...rest // Capture any other standard SVG props
}) => {
    // Determine the base and hover colors based on defaultStyle
    const primaryColorClass = defaultStyle
        ? "text-theme-red-coral"
        : "text-sky-600";
    
    const hoverActiveClasses = defaultStyle
        ? "group-hover:text-theme-red-coral hover:text-theme-red-coral active:text-theme-red-coral"
        : "group-hover:text-sky-600 hover:text-sky-600 active:text-sky-600";

    // Build the final Tailwind CSS class string
    const finalClassName = `${className} ${
        active ? primaryColorClass : ""
    ...
    return (
        <svg
            ...
            {...rest} // Spread rest props (like width, height, etc.)
        >
            {svgTypes[type]}
        </svg>
    );
